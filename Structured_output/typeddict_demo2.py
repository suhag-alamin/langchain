...
class ResumeAnalysis(TypedDict):
    candidate_name: Annotated[str,
                              "Extract the name of the Candidate from the resume"]
    key_skills: Annotated[list[str],
    ...
    experience_level: Annotated[Literal["entry-level", "mid-level",
                                        "senior-level"], "Classify the Candidate's Experience level"]

    strengths: Annotated[Optional[list[str]],
                         "List the candidate's major strengths"]

    weaknesses: Annotated[Optional[list[str]],
                          "List the candidate's possible weaknesses or ares for improvement"]
...
result = structured_model.invoke(
    """Suhag Al Amin. I am a Computer Science graduate with two years
